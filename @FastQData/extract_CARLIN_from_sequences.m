function [SEQ_trimmed, read_SEQ_trimmed, masks, trim_loc] = extract_CARLIN_from_sequences(SEQ_raw, read_SEQ_raw, cfg, CARLIN_def)
    ...
    % consites = G + (sgRNA sequences to 7 bp before PAM)
    ...
    fprintf('From %d reads, found (CARLIN_match,5_prime,3_prime,2nd_structure,valid_structure,length,no_N,all) sequences (%d,%d,%d,%d,%d,%d,%d,%d) times\n', ...
