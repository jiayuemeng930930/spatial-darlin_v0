function [CB, read_CB, UMI, read_UMI, SEQ, read_SEQ, linker,polyA, QC, Nreads] = parse_slideSeq_fastq(fastq_file)
    ...
    QC  = cell(Nfastqs,1);
    SEQ = cell(Nfastqs,1);
    linker = cell(Nfastqs,1);
    polyA = cell(Nfastqs,1);
    ...
    for i = 1:Nfastqs
        ...
        linker{i} = cell(Nreads{i,1},1); 
        polyA{i} = cell(Nreads{i,1},1);

        [~, CB{i}(:,1), QC{i}(:,1)] = fastqread(fastq_file{i,1});
        
        [CB{i}, UMI{i}, linker{i}, polyA{i}, QC{i}] = SpatialFastQData.parse_slideSeq_provenance(CB{i}, QC{i});

        %[QC{i}] = cellfun(@(x) deal([x(1:8) x(27:40)]), QC{i}, 'un', false);
