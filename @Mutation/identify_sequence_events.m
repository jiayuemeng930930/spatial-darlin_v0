function [mut_list, bp_event, seq, ref_seq, ref_mask] = identify_sequence_events(CARLIN_def, aligned)
    ...
    assert(length(ref_mask) == CARLIN_def.width.CARLIN);
    which_bp = zeros(size(ref_seq));
    which_bp(ref_mask) = [1:CARLIN_def.width.CARLIN];
    ...
    mut_list = {};
    if (~isempty(del))
        mut_list = arrayfun(@(bi,ei) Mutation(CARLIN_def, 'D', which_bp(bi), which_bp(ei), ...
    ...
    is = ismembc(frag_ind, CARLIN_def.bounds.consites(:,2));
    bp_event(frag_ind(is)+1) = 'I';
    bp_event(frag_ind(~is))  = 'I';
    
    if (~isempty(frag_ind(is)))
        mut_list = [mut_list; arrayfun(@(i) Mutation(CARLIN_def, 'I', i+1, i+1, ...
    ...
    if (~isempty(frag_ind(~is)))
        mut_list = [mut_list; arrayfun(@(i) Mutation(CARLIN_def, 'I', i, i, ...                                       
    ...
    if (ref_mask(1)~=1)
        bp_event(1) = 'I';
        mut_list = [mut_list; {Mutation(CARLIN_def, 'I', 1, 1, ...                                       
                               seq(1:ref_mask(1)), ...
    ...
    if (ref_mask(end)~=length(seq))
        bp_event(end) = 'I';
        mut_list = [mut_list; {Mutation(CARLIN_def, 'I', CARLIN_def.width.CARLIN, CARLIN_def.width.CARLIN, ...                                       
    ...
    mut_list = [mut_list; arrayfun(@(i) Mutation(CARLIN_def, 'M', i, i, seq(ref_mask(i)), CARLIN_def.seq.CARLIN(i)), find(bp_event=='M'), 'un', false)'];
