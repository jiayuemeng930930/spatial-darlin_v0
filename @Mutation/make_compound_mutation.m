function m = make_compound_mutation(CARLIN_def, s, e, in_seq, ref_seq)
    ...
    if (match_left+match_right < min_len)    
        ...
        m = Mutation(CARLIN_def, 'C', s+match_left, e-match_right, in_seq, ref_seq);
    else
        if (match_left+match_right > min_len)
        ...
        if (match_left+match_right == length(in_seq))
            ...
            m = Mutation(CARLIN_def, 'D', s+match_left, e-match_right, pad('', length(insert), '-'), insert);
        else
            if (match_left > 0)
                insert = in_seq(match_left:end-match_right);
                m = Mutation(CARLIN_def, 'I', s+match_left-1, e-match_right, insert, pad(ref_seq(match_left), length(insert), 'right', '-'));
            else
                insert = in_seq(1:end-match_right+1);
                m = Mutation(CARLIN_def, 'I', s, e-match_right+1, insert, pad(ref_seq(end-match_right+1), length(insert), 'left', '-'));
            end
        end
    end
end