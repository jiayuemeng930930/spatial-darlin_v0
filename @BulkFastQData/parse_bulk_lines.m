function [UMI, read_UMI, SEQ, read_SEQ, QC, trim_loc] = parse_bulk_lines(SEQ, read_SEQ, QC, cfg, trim_loc, CARLIN_def)
    ...
    if ((cfg.UMI.location == 'L') == (cfg.read_perspective.ShouldReverse == 'N'))
        end_loc = trim_loc.head_after_trim_5_primer-length(CARLIN_def.seq.Primer5);
        ...
        start_loc = trim_loc.tail_after_trim_3_primer+length(CARLIN_def.seq.Primer3);
    ...
    % Reorient UMIs so that what's stored is the same as what's read
    % left-to-right in the FASTQ
