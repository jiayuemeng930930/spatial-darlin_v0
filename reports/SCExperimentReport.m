classdef SCExperimentReport < ExperimentReport
    ...
    methods (Access = public)
        ...
        function obj = SCExperimentReport(CARLIN_def, alleles, allele_colony, N, reads)            
    ...
    methods (Static)
        ...
        function obj = create(CARLIN_def, CB_collection_denoised, CB_collection, CB_denoise_map, CB_called_allele, FQ, threshold, ref_CBs)
            ...
            temp = {CB_collection_denoised.CBs.UMIs}';
            ...
            N.uncleaned_tags = length(CB_collection.CBs);
            ...
            [is, where] = ismember({CB_called_allele.CB}', {CB_collection_denoised.CBs.CB}');
            assert(all(is));
            reads.called_tags_total  = cb_weights(where);
            reads.called_tags_allele = zeros(length(where),1);
            
            for i = 1:length(where)
                reads.called_tags_allele(i) = sum(arrayfun(@(j) sum(CB_collection_denoised.CBs(where(i)).UMIs(j).SEQ_weight ...
            ...
            obj = SCExperimentReport(CARLIN_def, alleles, allele_colony, N, reads);
        ...
        function obj = loadobj(s)
            ...
            assert(isa(s, 'SCExperimentReport'), 'loadobj only defined for existing SCExperimentReport objects');
            fprintf('Reformatting legacy SCExperimentReport object\n');
            CARLIN_def = ExperimentSummary.get_legacy_amplicon(s);            
            obj = SCExperimentReport(CARLIN_def, s.alleles, s.allele_colony, s.N, s.reads);
